...
def big_winning():
    ...
    for x in range(0,50):
        if b.find(id=("Lotto649Control_history_dlQuery_No1_"+str(x)))==None:
            ...
            dict_key.append(b.find(id=("Lotto649Control_history_dlQuery_L649_DrawTerm_"+str(x))).text)
        ...
        dict_value.append(w)
    m=dict(zip(dict_key,dict_value)) #中獎開獎日/兌獎日/號碼/特別號
###爬蟲一個月###
    ...
    def win(): #兌獎系統
        d_list=[n3.get(),n4.get(),n5.get(),n6.get(),n7.get(),n8.get()]
        ...
        if (len(n1.get())!=9):#判斷期別值是否有誤
            ...
            exist()
        elif (len(d)!=6):#判斷s串列號碼是有6個
            messagebox.showwarning("warn!","輸入值有誤、有少或重複！！！")
            exist()
        elif n1.get() in m:
            g=m.get(n1.get()) #中獎普通號
            del g[0:2] #刪除開獎兌獎日期
            f=g[6] #中獎特別號
            del g[6] 
             #使用者號碼
    ...
    label5= Label(window,text="大樂透對獎系統",fg="black",bg='#B8B8DC',
    ...
    label5= Label(window,text="ex:111000011",fg="black",bg='#B8B8DC',
